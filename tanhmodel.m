function o = tanhmodel(par, var, fixpar, array)
...
for subj = 1:length(array)
    ...
    for session = 1:size(z,1)
        ...
        for t = 2:(T-1)    
            ...
            mu(t) = tanh(beta*Y_);
            xi(t) = pi_par*(abs(mu(t)*cos(TargetPos_pi(Z(t),Z(t-1))))) + (1-pi_par)*mu(t);
            ALT(t) = X(t) + X(t-1) - 2*X(t)*X(t-1);
            omega(t) = (1-ALT(t))*xi(t) + (1 - xi(t))*omega(t-1);
            omega(t) = max([0.01 min([omega(t) 0.99])]);
